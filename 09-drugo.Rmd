...
1.  Sliko shranimo v vektorskem formatu `.pgf`. To lahko naredimo s klicem funkcije `plt.savefig(pot.pgf)`. Sliko nato dodamo v LaTeX dokument z `\input{pot.pgf}`, pred tem pa na začetku datoteke uvozimo `\usepackage{pgf}`. Prednost vektorske slike je, da slika ohrani ostrino ne glede na to, koliko povečana je, in da so fonti številk in oznak na sliki enaki kot v LaTeXu ter prave velikosti. Da se tudi nastaviti, da je širina slike npr. ravno polovico širine dokumenta. Vse skupaj je opisano tu: <https://timodenk.com/blog/exporting-matplotlib-plots-to-latex> Za grafe z veliko točkami (\>1000) pa je bolje uporabiti `.png`, sicer je stavljenje dokumenta z LaTeXom lahko zelo počasno.
2.  Sliko shranimo v rastrskem formatu `.png`. To lahko naredimo s klicem funkcije `plt.savefig(pot.png, dpi=300)`. Parameter `dpi` (dots per inch) dolča resolucijo slike. Ponavadi zadošča 300. Sliko nato dodamo v LaTeX dokument z `\includegraphics{pot.png}`. Privzeto fonti ne bodo taki kot v LaTeXu, kar ni najlepše. Prave fonte lahko nastavimo z
...
Obširen tečaj računalništva. Vzame nekaj ur, priporočam predvsem prvih 21 videov. <https://www.youtube.com/watch?v=tpIctyqH29Q&list=PLH2l6uzC4UEW0s7-KewFLBC1D0l6XRfye&index=1>
