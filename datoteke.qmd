...
Za delo s potmi je na voljo knjižnica `os.path`, kjer so pogosto
...
Datoteko odpremo v načinu za pisanje `mode="w"` in uporabimo funkcijo
`write()`, ki zapiše niz v datoteko. Znak `\n` pomeni novo vrstico. Če
želimo zapisati znak `\` moramo v Pythonu napisati `\\`. Več o uporabi
posebnih znakov v Pythonu:
